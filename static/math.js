...
class rnn{
  ...
  learn(x, o, win){
    ...
    for(let i=0; i<x.length; i++){
      input.push(Matrix.fromArray(x[i]))
    }
    for(let i=0; i<x.length; i++){
      output.push(Matrix.fromArray(o[i]))
    }
    let units = nn.feedforward(x)
    let s = units.s
    let u = units.u
    let y = units.y
    let v = units.v
    ...
    let dU = new Array()
    let dV = new Array()
    for(let i=0; i<tl-win; i++){
      for(let j=0; j<win; j++){
        oe.push(Matrix.subtract(y[i+j], output[i+j]))
        go.push(Matrix.map(y[i+j], sigmoid).multiply(oe[i+j]).multiply(this.lr))
        // gs calculate
        dU.push(Matrix.multiply(go[i+j], Matrix.transpose(this.U)))
        he.push(Matrix.multiply(Matrix.transpose(this.U), oe[i+j]))
        gh.push(Matrix.map(s[i+j], sigmoid).multiply(he[i+j]).multiply(this.lr))
      }
    }
  }
  generateText(x,tp, func){
    ...
    r.open('GET', "/word2vec-js-master/word2vec/data/wordvecs1000.json")
    let U  = this.U
    let H = this.H
    let V = this.V
